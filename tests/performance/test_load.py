...
class TestAPIPerformance:
    ...
    async def test_deals_list_response_time(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                "https://lux.kushnir.cloud/api/v2/deals?limit=50", headers=headers
            ) as resp:
            ...
            assert (
                elapsed_ms < 500
            ), f"Response time {elapsed_ms}ms exceeds 500ms target"
    ...
    async def test_concurrent_requests(self):
        ...
        async def make_request():
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "https://lux.kushnir.cloud/api/v2/deals", headers=headers
                ) as resp:
                    return resp.status
...
class TestLoadTesting:
    ...
    async def test_500_concurrent_connections(self):
        ...
        assert (
            success_rate >= 0.95
        ), f"Success rate {success_rate*100}% below 95% target"
...
class TestCacheHitRates:
    ...
    async def test_cache_hit_rate(self):
        ...
        async with aiohttp.ClientSession() as session:
            start1 = datetime.now()
            async with session.get(
                "https://lux.kushnir.cloud/api/v2/analytics/dashboard", headers=headers
            ) as resp:
                await resp.json()
