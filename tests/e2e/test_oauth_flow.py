...
def test_api_me_with_auth_header_new_user(client: TestClient):
    ...
    response = client.get("/api/me", headers={"X-Auth-Request-Email": test_email})
...
def test_api_me_with_auth_header_existing_user(client: TestClient):
    ...
    response1 = client.get("/api/me", headers={"X-Auth-Request-Email": test_email})
    user_id_1 = response1.json()["id"]

    # Second request returns same user
    response2 = client.get("/api/me", headers={"X-Auth-Request-Email": test_email})
...
def test_api_me_admin_user(client: TestClient):
    ...
    response = client.get("/api/me", headers={"X-Auth-Request-Email": admin_email})
...
def test_logout_endpoint(client: TestClient):
    ...
    # First authenticate
    client.get("/api/me", headers={"X-Auth-Request-Email": test_email})

    # Then logout
    response = client.post(
        "/api/sessions/logout", headers={"X-Auth-Request-Email": test_email}
    )
...
def test_active_sessions_endpoint(client: TestClient):
    ...
    # Get sessions
    response = client.get("/api/sessions", headers={"X-Auth-Request-Email": test_email})
...
def test_logout_all_sessions(client: TestClient):
    ...
    # Logout all
    response = client.post(
        "/api/sessions/logout-all", headers={"X-Auth-Request-Email": test_email}
    )
...
def test_multiple_different_users(client: TestClient):
    ...
    user_ids = []
    for email in emails:
        response = client.get("/api/me", headers={"X-Auth-Request-Email": email})
        assert response.status_code == 200
...
async def test_oauth_flow_simulation(client: TestClient):
    ...
    response = client.get(
        "/api/me", headers={"X-Auth-Request-Email": "oauth_user@example.com"}
    )
    assert response.status_code == 200
    ...
    response = client.get(
        "/api/sessions", headers={"X-Auth-Request-Email": "oauth_user@example.com"}
    )
    assert response.status_code == 200
