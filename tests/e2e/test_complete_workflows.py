...
def admin_user(db: Session):
    ...
    user = User(email="admin@test.local", name="Test Admin", role="admin", active=True)
    db.add(user)
    db.commit()
    return user
...
def analyst_user(db: Session):
    ...
    user = User(
        email="analyst@test.local", name="Test Analyst", role="analyst", active=True
    )
    db.add(user)
    db.commit()
    return user
...
class TestDealWorkflow:
    ...
    async def test_deal_creation(self, async_client, analyst_user, deal_data):
        ...
        response = await async_client.post(
            "/api/v2/deals", json=deal_data, headers=headers
        )
...
class TestAdminWorkflow:
    ...
    async def test_rbac_permission_assignment(self, async_client, admin_user):
        ...
        payload = {
            "user_id": 999,
            "role": "buyer_manager",
            "permissions": ["deal.view", "deal.create", "buyer.manage"],
        }
...
class TestWebSocketUpdates:
    ...
    async def test_deal_update_broadcast(self, async_client, analyst_user):
        ...
        async with async_client.websocket_connect(
            "/ws/deals", headers={"Authorization": f"Bearer {analyst_user.id}"}
        ) as websocket:
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_deal_prevention(
        self, async_client, analyst_user, deal_data
    ):
