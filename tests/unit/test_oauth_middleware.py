...
class TestOAuthMiddleware:
    ...
    def test_user_info_endpoint_returns_email(self, client):
        ...
        response = client.get(
            "/api/auth/user", headers={"X-Auth-Request-Email": "test@example.com"}
        )
        assert response.status_code == 200
    ...
    def test_me_endpoint_alias(self, client):
        ...
        response = client.get(
            "/api/auth/me", headers={"X-Auth-Request-Email": "alice@example.com"}
        )
        assert response.status_code == 200
    ...
    def test_oauth_headers_preserved_in_response(self, client):
        ...
        response = client.get(
            "/api/dashboard", headers={"X-Auth-Request-Email": "user@example.com"}
        )
        assert response.status_code == 200
...
class TestAuthDependency:
    ...
    def test_auth_dependency_populated(self, client):
        ...
        response = client.get(
            "/api/profile", headers={"X-Auth-Request-Email": "user@example.com"}
        )
        assert response.status_code == 200
