...
class TestUserRolesRelationship:
    ...
    def test_user_roles_relationship_defined(self):
        ...
        assert hasattr(
            User, "user_roles"
        ), "User model must have user_roles relationship"
    ...
    def test_user_roles_relationship_foreign_keys_specified(self):
        ...
        assert (
            user_roles_rel.direction.name == "ONETOMANY"
        ), "user_roles should be one-to-many"
        assert (
            user_roles_rel.mapper.class_.__name__ == "UserRole"
        ), "Should map to UserRole"
    ...
    def test_user_role_model_has_dual_foreign_keys(self):
        ...
        assert (
            fk_targets.count("users") >= 2
        ), "UserRole should have multiple FKs to users table"
...
class TestSchemaInitialization:
    ...
    def test_create_all_creates_all_tables(self, test_db_engine):
        ...
        assert (
            created_tables == expected_tables
        ), f"Tables not created: {expected_tables - created_tables}"
    ...
    def test_orphaned_index_cleanup_logic(self, test_db_engine):
        ...
        with test_db_engine.begin() as conn:
            try:
                ...
                conn.execute(
                    text("CREATE INDEX IF NOT EXISTS test_orphaned_idx ON users(email)")
                )
            except:
