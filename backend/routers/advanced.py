...
from backend.database.models import Deal, User
...
class ScheduledDealRequest(BaseModel):
    ...
    target_buyers: Optional[List[int]] = Field(
        default=None, description="Specific buyer IDs"
    )
...
class DealBatchRequest(BaseModel):
    ...
    deal_ids: List[int] = Field(
        ..., description="Deal IDs to batch", min_items=1, max_items=100
    )
...
class PredictiveScoreResponse(BaseModel):
    ...
    base_score: float = Field(
        ..., ge=0, le=100, description="Base market potential score"
    )
...
async def predict_deal_score(
    req: PredictiveScoreRequest, current_user: User = Depends(get_current_user)
):
    """
    ...
    condition_mult = {"excellent": 1.15, "good": 1.0, "fair": 0.85, "poor": 0.6}.get(
        req.condition, 1.0
    )
...
async def suggest_intelligent_routing(
    ...
    logger.info(
        "Intelligent routing suggestions generated",
        extra={
            "deal_id": req.deal_id,
...
async def get_batch_status(
    ...
    return {
        "batch_id": batch_id,
        "status": "processing",
        "progress": {"completed": 34, "total": 100, "percentage": 34},
        "active_workers": 4,
