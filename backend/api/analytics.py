...
async def get_dashboard_metrics(
    ...
    if not check_permission(db, user.id, Permission.READ_ANALYTICS):
        raise HTTPException(
            status_code=403, detail=f"Permission '{Permission.READ_ANALYTICS}' required"
        )
    ...
    recent_deals = (
        db.query(func.count(Deal.id)).filter(Deal.created_at >= cutoff_date).scalar()
        or 0
    )
    ...
    for status in ["scanning", "scored", "bidding", "won", "routed", "closed"]:
        count = (
            db.query(func.count(Deal.id)).filter(Deal.status == status).scalar() or 0
        )
...
async def get_deals_analytics(
    ...
    avg_margin = (
        sum([float(d.estimated_margin or 0) for d in deals]) / total if total > 0 else 0
    )
    ...
    for key in make_breakdown:
        make_breakdown[key]["avg_score"] = (
            make_breakdown[key]["total_score"] / make_breakdown[key]["count"]
        )
...
async def get_buyers_analytics(
    ...
    for buyer in buyers:
        success_rate = (
            (buyer.success_count / buyer.contact_count * 100)
            if buyer.contact_count > 0
            else 0
        )
    ...
    return {
        ...
        "avg_success_rate": (
            sum([b["success_rate"] for b in buyer_metrics]) / len(buyer_metrics)
            if buyer_metrics
            else 0
        ),
    }
