...
# Error metrics
ERROR_COUNT = Counter(
    "errors_total", "Total errors by type", ["error_type", "endpoint"]
)

EXCEPTION_COUNT = Counter("exceptions_total", "Total exceptions", ["exception_type"])
...
WEBSOCKET_MESSAGE_COUNT = Counter(
    "websocket_messages_total", "Total WebSocket messages", ["message_type"]
)

# Cache metrics
CACHE_HIT_RATE = Gauge("cache_hit_rate", "Cache hit rate percentage")

CACHE_MEMORY_USAGE = Gauge("cache_memory_usage_bytes", "Redis memory usage")

# Business metrics
DEALS_CREATED = Counter("deals_created_total", "Total deals created", ["source"])

DEALS_APPROVED = Counter("deals_approved_total", "Total deals approved")

DEALS_WON = Counter("deals_won_total", "Total deals won")

DEAL_REVENUE = Counter("deal_revenue_total", "Total deal revenue", ["currency"])

AVERAGE_DEAL_MARGIN = Gauge(
    "average_deal_margin_percent", "Average deal margin percentage"
)
...
class HealthChecker:
    ...
    def get_status(self, db_session, redis_client) -> HealthStatus:
        ...
        return HealthStatus(status=overall, checks=self.checks)
...
class AlertManager:
    ...
    def check_thresholds(self, metrics: Dict[str, float]) -> List[Dict]:
        ...
        for threshold in ALERT_THRESHOLDS:
            ...
            if is_triggered:
                alert = self._create_alert(threshold, current_value, metrics)
    ...
    def _create_alert(
        self, threshold: AlertThreshold, current_value: float, metrics: Dict
    ) -> Dict:
        """Create alert object."""
        return {
    ...
    def _send_alert(self, alert: Dict):
        ...
        with self.lock:
            ...
            logger.log(
                severity_level, f"ALERT: {alert['description']}", extra={"alert": alert}
            )
...
class SLATracker:
    ...
    def record_downtime(
        self, start: datetime, end: datetime, reason: str, component: str = "api"
    ):
    ...
    def get_uptime_percentage(self) -> float:
        ...
        downtime = sum(e["duration_seconds"] for e in self.downtime_events)
